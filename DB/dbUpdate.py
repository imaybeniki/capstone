...
with open('/home/christopher/Downloads/2017-Q1-Trips-History-Data.csv') as csvfile:
    with open('/home/christopher/BikeShareFormatted.csv','w') as csvWrite:
        ...
        for row in readCSV:
            ...
            addressCheck=(row[rowNumber])
            
            if addressCheck in prevAddr:
                print(addressCheck, "address already used, skipping value")
                continue
            
            prevAddr.append(addressCheck)
            
        
                 
            location = geolocator.geocode(row[rowNumber])
    ...
    #         prevAddr.append(location)
           ...
           # print("INSERT INTO LOCATIONS( lOCATION_ADDRESS, LOCATION_CITY, lOCATION_STATE,LOCATION_ZIP,LOCATION_LAT,LOCATION_LONG,CAPACITY,OCCUPIED,WEIGHT) VALUES")    
            print(k,row[rowNumber],city,state,intZip, lat, longe, occ, cap, weight, flag)
            k=k+1     
            ...
            writedata=(retAddr,city,state,str(intZip),str(lat),str(longe),str(cap),str(occ),str(weight),flag)
